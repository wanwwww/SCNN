...
void PoolingUnit::cycle() {
    ...
    // 1. 接收数据
    DataPackage* pck_received;  // 计算结束之后delete
    ...
    if(!this->input_fifo->isEmpty()){
        ...
        if(this->current_num == (this->operate_num-1)){
            DataPackage* pck_result = new DataPackage(this->result, pck_received->channel_num, pck_received->retrieve_num, this->location);
    ...
    if(!this->output_fifo->isEmpty()){
        ...
        DataPackage* pck_result = this->output_fifo->pop(); 
